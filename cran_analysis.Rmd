...
big_pkgs <- df_cran_pkg_logs %>%
  ...
  summarise(mean_download  = mean(nb_download),
            median_download = median(nb_download)) %>% 
...
# have view
views_download %>% 
  ggplot(aes(x = median_download, fill = have_view)) +
  geom_histogram(position = "dodge", bins = 10) +
  theme_minimal()

views_download %>% 
  arrange(desc(median_download)) %>% 
  select(pkg_name, views, have_view, median_download) %>% 
  head(30) %>% 
  flextable()
...
views_download %>% 
  ...
  group_by(views) %>% 
  summarise(median_download = mean(median_download, na.rm = TRUE)) %>% 
  arrange(desc(median_download)) %>% 
  flextable()
```
